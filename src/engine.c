...
void refresh_quick_totals() {
  for (int land_idx = 0; land_idx < LANDS_COUNT; land_idx++) {
    ...
    for (int player_idx = 0; player_idx < PLAYERS_COUNT; player_idx++) {
      uint8_t* cur_units_land_player_total = other_land_units_ptr[player_idx][land_idx];
      *cur_units_land_player_total = 0;
      for (int unit_idx = 0; unit_idx < LAND_UNIT_TYPES; unit_idx++) {
        *cur_units_land_player_total += other_land_units_ptr[player_idx][land_idx][unit_idx];
      }
    }
  }
  for (int sea_idx = 0; sea_idx < SEAS_COUNT; sea_idx++) {
    for (int player_idx = 0; player_idx < PLAYERS_COUNT; player_idx++) {
      uint8_t* cur_units_sea_player_total = other_sea_units_ptr[player_idx][sea_idx];
      *cur_units_sea_player_total = 0;
      for (int unit_idx = 0; unit_idx < SEA_UNIT_TYPES; unit_idx++) {
        *cur_units_sea_player_total += other_sea_units_ptr[player_idx][sea_idx][unit_idx];
      }
    }
  }
}
...
void refresh_cache() {
  ...
  for (int land_idx = 0; land_idx < LANDS_COUNT; land_idx++) {
    ...
    for (int enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
      ...
      enemy_units_count[land_idx] += units_land_player_total[ememy_player_idx][land_idx];
    }
  }
  for (int src_land = 0; src_land < LANDS_COUNT; src_land++) {
    for (int dst_land = 0; dst_land < LANDS_COUNT; dst_land++) {
...
#ifdef DEBUG
  for (int player_idx = 0; player_idx < PLAYERS_COUNT; player_idx++) {
    for (int land_idx = 0; land_idx < AIRS_COUNT; land_idx++) {
      if (units_land_player_total[player_idx][land_idx] < 0) {
        DEBUG_PRINT("units_land_player_total[player_idx][land_idx] < 0");
      }
    }
    for (int sea_idx = 0; sea_idx < SEAS_COUNT; sea_idx++) {
      if (units_sea_player_total[player_idx][sea_idx] < 0) {
        DEBUG_PRINT("units_sea_player_total[player_idx][sea_idx] < 0");
      }
    }
  }
  status_message_id++;
  strcat(printableGameStatus, "Status message id: ");
  sprintf(threeCharStr, "%d", status_message_id);
...
void setPrintableStatusLands() {
  ...
  for (int land_idx = 0; land_idx < LANDS_COUNT; land_idx++) {
    ...
    if (units_land_player_total[0][land_idx] > 0) {
      ...
      uint8_t* other_land_units_here = other_land_units_0[land_idx];
      for (int land_unit_idx = 0; land_unit_idx < LAND_UNIT_TYPES; land_unit_idx++) {
        ...
        if (unit_count > 0) {
          ...
          strcat(printableGameStatus, threeCharStr);
          uint8_t* units_here = units_land_ptr[land_idx][land_unit_idx];
    ...
    for (int player_idx = 1; player_idx < PLAYERS_COUNT; player_idx++) {
      ...
      uint8_t* other_land_units_here = other_land_units_ptr[player_idx][land_idx];
#ifdef DEBUG
...
void setPrintableStatusSeas() {
  ...
  for (int sea_index = 0; sea_index < SEAS_COUNT; sea_index++) {
    ...
    for (int player_idx = 1; player_idx < PLAYERS_COUNT; player_idx++) {
      ...
      for (int sea_unit_idx = 0; sea_unit_idx < SEA_UNIT_TYPES - 1; sea_unit_idx++) {
        uint8_t unit_count = other_sea_units_ptr[player_idx][sea_index][sea_unit_idx];
        if (unit_count > 0) {
          strcat(printableGameStatus,
...
void add_valid_land_moves(uint8_t* valid_moves, uint8_t* valid_moves_count, uint8_t src_land,
  ...
  if (moves_remaining == 2) {
    ...
    int lands_within_1_move_count = LANDS_WITHIN_1_MOVE_COUNT[src_land];
...
void stage_transport_units() {
  ...
  for (uint8_t unit_type = TRANS_EMPTY; unit_type <= TRANS_1T; unit_type++) {
    ...
    for (uint8_t src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
      uint8_t* total_ships = &units_sea_ptr[src_sea][unit_type][staging_state];
      if (*total_ships == 0)
        continue;
...
void move_fighter_units() {
  ...
  for (int src_air = 0; src_air < AIRS_COUNT; src_air++) {
    ...
    while (*total_fighters > 0) {
      ...
      if (src_air == dst_air) {
        units_air_ptr[src_air][FIGHTERS][0] += *total_fighters;
        *total_fighters = 0;
        continue;
      }
      ...
      if (dst_air < LANDS_COUNT) {
        ...
        other_land_units_0[dst_air][FIGHTERS]++;
      ...
      if (src_air < LANDS_COUNT) {
        other_land_units_0[src_air][FIGHTERS]--;
...
void move_bomber_units() {
  ...
  for (int src_land = 0; src_land < LANDS_COUNT; src_land++) {
    ...
    while (*total_bombers > 0) {
      ...
      if (src_land == dst_air) {
        units_land_ptr[src_land][BOMBERS_LAND_AIR][0] += *total_bombers;
        *total_bombers = 0;
        continue;
      }
      if (dst_air < LANDS_COUNT) {
      ...
      other_land_units_0[src_land][BOMBERS_LAND_AIR]--;
      units_land_player_total[0][src_land]--;
      *total_bombers -= 1;
    }
  }
  ...
  for (int src_land = 0; src_land < LANDS_COUNT; src_land++) {
    for (int moves_remaining = MAX_MOVE_LAND[unit_type]; moves_remaining > 0; moves_remaining--) {
      ...
      while (*total_units > 0) {
        ...
        update_move_history(dst_air, src_land, valid_moves, &valid_moves_count);
        if (src_land == dst_air) {
          units_land_ptr[src_land][unit_type][0] += *total_units;
          *total_units = 0;
          continue;
        }
        if (dst_air >= LANDS_COUNT) {
        ...
        other_land_units_0[dst_air][unit_type]++;
        units_land_player_total[0][dst_air]++;
        other_land_units_0[src_land][unit_type]--;
        units_land_player_total[0][src_land]--;
        *total_units -= 1;
...
void move_transport_units() {
  DEBUG_PRINT("Entering move_transport_units");
  for (int unit_type = TRANS_1I; unit_type <= TRANS_1I_1T;
    ...
    for (int src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
      for (int cur_state = max_state; cur_state >= min_state; cur_state--) {
        uint8_t* total_ships = &units_sea_ptr[src_sea][unit_type][cur_state];
        if (*total_ships == 0)
          continue;
        ...
        while (*total_ships > 0) {
          ...
          if (src_air == dst_air) {
            units_sea_ptr[src_sea][unit_type][done_moving] += *total_ships;
            *total_ships = 0;
            continue;
          }
...
void move_subs() {
  ...
  for (int src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
    ...
    while (*total_subs > 0) {
      ...
      if (src_air == dst_air) {
        units_sea_ptr[src_sea][SUBMARINES][SUB_DONE_MOVING] += *total_subs;
        *total_subs = 0;
        continue;
      }
    ...
    for (int src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
      ...
      while (*total_ships > 0) {
        ...
        if (unit_type == DESTROYERS) {
          printf("DEBUG: src_sea: %d, dst_air: %d\n", src_sea, dst_air);
        }
#endif
        update_move_history(dst_air, src_air, valid_moves, &valid_moves_count);
...
void carry_allied_fighters(uint8_t src_sea, uint8_t dst_sea) {
  ...
  for (int other_player_idx = 0; other_player_idx < PLAYERS_COUNT - 1; other_player_idx++) {
    while (other_sea_units_ptr[other_player_idx][src_sea][FIGHTERS] > 0) {
      other_sea_units_ptr[other_player_idx][src_sea][FIGHTERS]--;
      other_sea_units_ptr[other_player_idx][dst_sea][FIGHTERS]++;
...
void resolve_sea_battles() {
  for (int src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
    ...
    bool defender_submerged = other_sea_units_0[src_sea][DESTROYERS] == 0;
    ...
    while (true) {
      ...
      setPrintableStatus();
      printf("%s\n", printableGameStatus);
      if (other_land_units_0[2][FIGHTERS] == 255) {
...
#endif
      uint8_t* units11 = &units_sea_ptr[src_sea][DESTROYERS][0];
      ...
      if (units_sea_ptr[src_sea][SUBMARINES][0] > 0) { // no blockade ships, only subs
        for (int enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
          int enemy_player_idx = enemies_0[enemy_idx];
          if (units_sea_player_total[enemy_player_idx][src_sea] -
                  other_sea_units_ptr[enemy_player_idx][src_sea][FIGHTERS] >
              0) {
            targets_exist = true;
            break;
          }
      ...
      if (other_sea_units_0[src_sea][FIGHTERS] + other_sea_units_0[src_sea][BOMBERS_SEA] >
                 0) { // no ships, only air
        for (int enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
          int enemy_player_idx = enemies_0[enemy_idx];
          if (units_sea_player_total[enemy_player_idx][src_sea] -
                  other_sea_units_ptr[enemy_player_idx][src_sea][SUBMARINES] >
              0) {
            targets_exist = true;
            break;
          }
      ...
      if (units_sea_player_total[0][src_sea] > 0) { // attacker only has transports
        for (int enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
          if (units_sea_player_total[enemies_0[enemy_idx]][src_sea] > 0) {
            targets_exist = true;
            break;
      ...
      if (!defender_submerged) {
        defender_damage = 0;
        for (int enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
          defender_damage += other_sea_units_ptr[enemies_0[enemy_idx]][src_sea][SUBMARINES];
        }
      ...
      if (sea_dist[src_sea][retreat] == 1 && enemy_units_count[retreat] == 0 &&
        ...
        for (uint8_t unit_type = TRANS_EMPTY; unit_type <= BS_DAMAGED; unit_type++) {
          units_sea_ptr[retreat][unit_type][0] += other_sea_units_0[src_sea][unit_type];
          other_sea_units_0[retreat][unit_type] += other_sea_units_0[src_sea][unit_type];
          units_sea_player_total[0][retreat] += other_sea_units_0[src_sea][unit_type];
          units_sea_player_total[0][src_sea] -= other_sea_units_0[src_sea][unit_type];
...
void remove_land_defenders(uint8_t src_land, uint8_t hits) {
  for (uint8_t unit_idx = 0; unit_idx < DEFENDER_LAND_UNIT_TYPES; unit_idx++) {
    for (uint8_t enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
      ...
      uint8_t* total_units =
          &other_land_units_ptr[enemy_player_idx][src_land][ORDER_OF_LAND_DEFENDERS[unit_idx]];
      if (*total_units > 0) {
#ifdef DEBUG
        printf(
        ...
        if (*total_units < hits) {
          hits -= *total_units;
          units_land_player_total[enemy_player_idx][src_land] -= *total_units;
          enemy_units_count[src_land] -= *total_units;
          *total_units = 0;
        } else {
          *total_units -= hits;
          units_land_player_total[enemy_player_idx][src_land] -= hits;
...
void remove_sea_defenders(uint8_t src_sea, uint8_t hits, bool defender_submerged) {
  ...
  for (uint8_t enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
    uint8_t enemy_player_idx = enemies_0[enemy_idx];
    uint8_t* total_battleships = &other_sea_units_ptr[enemy_player_idx][src_sea][BATTLESHIPS];
    uint8_t* total_bs_damaged = &other_sea_units_ptr[enemy_player_idx][src_sea][BS_DAMAGED];
  ...
  if (!defender_submerged) {
    for (uint8_t enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
      uint8_t enemy_player_idx = enemies_0[enemy_idx];
      uint8_t* total_units = &other_sea_units_ptr[enemy_player_idx][src_sea][SUBMARINES];
      if (*total_units > 0) {
        if (*total_units < hits) {
          hits -= *total_units;
          units_sea_player_total[enemy_player_idx][src_sea] -= *total_units;
          enemy_units_count[src_sea] -= *total_units;
          *total_units = 0;
        } else {
          *total_units -= hits;
          units_sea_player_total[enemy_player_idx][src_sea] -= hits;
  ...
  for (uint8_t unit_idx = 1; unit_idx < DEFENDER_SEA_UNIT_TYPES; unit_idx++) {
    for (uint8_t enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
      uint8_t enemy_player_idx = enemies_0[enemy_idx];
      uint8_t* total_units =
          &other_sea_units_ptr[enemy_player_idx][src_sea][ORDER_OF_SEA_DEFENDERS[unit_idx]];
      if (*total_units > 0) {
        if (*total_units < hits) {
          hits -= *total_units;
          units_sea_player_total[enemy_player_idx][src_sea] -= *total_units;
...
void unload_transports() {
  for (uint8_t unit_type = TRANS_1I; unit_type <= TRANS_1I_1T; unit_type++) {
    ...
    clear_move_history();
    for (int src_sea = 0; src_sea < SEAS_COUNT; src_sea++) {
      uint8_t* total_units = &units_sea_ptr[src_sea][unit_type][unloading_state];
      if (*total_units == 0)
        continue;
      uint8_t src_air = src_sea + LANDS_COUNT;
      uint8_t valid_moves[AIRS_COUNT];
      ...
      while (*total_units > 0) {
        ...
        update_move_history(dst_air, src_sea, valid_moves, &valid_moves_count);
        if (src_air == dst_air) {
          units_sea_ptr[src_sea][unit_type][0] += *total_units;
          *total_units = 0;
          continue;
        }
        if (enemy_units_count[dst_air] > 0) {
        ...
        other_land_units_0[dst_air][unload_cargo1]++;
        units_land_player_total[0][dst_air]++;
        units_sea_ptr[src_sea][TRANS_EMPTY][0]++;
        other_sea_units_0[src_sea][TRANS_EMPTY]++;
        other_sea_units_0[src_sea][unit_type]--;
        units_sea_player_total[0][src_sea]--;
        *total_units -= 1;
        if (unit_type > TRANS_1T) {
          ...
          other_land_units_0[dst_air][unload_cargo2]++;
          units_land_player_total[0][dst_air]++;
        }
      }
    }
...
void resolve_land_battles() {
  ...
  for (uint8_t src_land = 0; src_land < LANDS_COUNT; src_land++) {
    ...
    snprintf(debug_message, sizeof(debug_message), "Resolve land combat in: %d, Name: %s", src_land,
             LANDS[src_land].name);
    DEBUG_PRINT(debug_message);
    setPrintableStatus();
    printf("%s\n", printableGameStatus);
#endif
    int* units_land_player_total_0_src_land = &units_land_player_total[0][src_land];
    ...
    uint8_t attacker_hits;
    // only bombers exist
    uint8_t* other_land_units_ptr_0_src_land = other_land_units_ptr[0][src_land];
    uint8_t* bombers_count = &other_land_units_ptr_0_src_land[BOMBERS_LAND_AIR];
    if (*bombers_count > 0 && *other_land_units_ptr_0_src_land == *bombers_count) {
      if (*factory_hp[src_land] > -*factory_max[src_land]) {
        ...
        uint8_t defender_hits =
            ...
            (RANDOM_NUMBERS[random_number_index++] % 6 < defender_damage % 6 ? 1 : 0);
        if (defender_hits > 0) {
          uint8_t* units_land_ptr_src_land_bombers = units_land_ptr[src_land][BOMBERS_LAND_AIR];
    ...
    uint8_t* other_land_units_0_src_land = other_land_units_0[src_land];
    // bombard_shores
    if (bombard_max[src_land] > 0) {
      ...
      for (uint8_t unit_type = BS_DAMAGED; unit_type >= CRUISERS; unit_type--) {
        for (uint8_t sea_idx = 0; sea_idx < LAND_TO_SEA_COUNT[src_land]; sea_idx++) {
          ...
          uint8_t* total_bombard_ships = units_sea_ptr[src_sea][unit_type];
    ...
    while (true) {
      ...
      if (enemy_units_count[src_land] == -1) {
        DEBUG_PRINT("No enemies remain");
      }
      ...
      attacker_hits = (attacker_damage / 6) +
                      (RANDOM_NUMBERS[random_number_index++] % 6 < attacker_damage % 6 ? 1 : 0);
      defender_damage = 0;
      printf("Enemy Count: %d", enemy_units_count[src_land]);
      for (uint8_t enemy_idx = 0; enemy_idx < enemies_count_0; enemy_idx++) {
      ...
      if (LAND_DIST[src_land][retreat] == 1 && enemy_units_count[retreat] == 0 &&
        ...
        for (uint8_t unit_type = INFANTRY; unit_type <= TANKS; unit_type++) {
          int total_units = units_land_ptr[src_land][unit_type][0];
          units_land_ptr[retreat][unit_type][0] += total_units;
          other_land_units_0[retreat][unit_type] += total_units;
          units_land_player_total[0][retreat] += total_units;
          other_land_units_0[src_land][unit_type] -= total_units;
          *units_land_player_total_0_src_land -= total_units;
    ...
    for (int src_air = 0; src_air < AIRS_COUNT; src_air++) {
      ...
      while (*total_bomber_count > 0) {
        ...
        units_air_ptr[dst_air][BOMBERS_LAND_AIR][0]++;
          units_land_player_total[0][dst_air]++;
          other_land_units_0[dst_air][BOMBERS_LAND_AIR]++;
        if (src_air < LANDS_COUNT) {
          units_land_player_total[0][src_air]--;
          other_land_units_0[src_air][BOMBERS_LAND_AIR]--;
        } else {
          uint8_t src_sea = src_air - LANDS_COUNT;
          units_sea_player_total[0][src_sea]--;
          other_land_units_0[src_sea][BOMBERS_LAND_AIR]--;
        }
        *total_bomber_count -= 1;
      }
    }
...
void rotate_turns() {
  ...
  for (uint8_t dst_sea = 0; dst_sea < SEAS_COUNT; dst_sea++) {
    memcpy(&data.other_sea_units[PLAYERS_COUNT - 2][dst_sea], &other_sea_units_temp[dst_sea],
           SEA_UNIT_TYPES - 1);
...
double get_score() {
  ...
  for (int player_idx = 0; player_idx < PLAYERS_COUNT; player_idx++) {
    ...
    for (int land_idx = 0; land_idx < LANDS_COUNT; land_idx++) {
      for (int unit_type = 0; unit_type < LAND_UNIT_TYPES; unit_type++) {
        score += other_land_units_ptr[player_idx][land_idx][unit_type] * COST_UNIT_LAND[unit_type];
      }
    }
    for (int sea_idx = 0; sea_idx < SEAS_COUNT; sea_idx++) {
      for (int unit_type = 0; unit_type < SEA_UNIT_TYPES; unit_type++) {
        score += other_sea_units_ptr[player_idx][sea_idx][unit_type] * COST_UNIT_SEA[unit_type];
      }
    }
    if (is_allied_0[player_idx]) {
      allied_score += score;
  ...
  return (double)allied_score / (double)enemy_score;
}